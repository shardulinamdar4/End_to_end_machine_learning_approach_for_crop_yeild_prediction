...
import pickle
X_hr = pd.read_csv(r'C:\Users\user\Desktop\final\Coimbatore.csv')#importing the dataset
print(X_hr)
X_finite = X_hr[np.isfinite(X_hr["X1"])]        #filling the empty cells with median
...
Xn = X_finite
print(Xn)
y = Xn["Y"]                         #Declaring the features
X = Xn[["X1", "X2", "X3", "X4"]]    #Declaring the label
plt.figure(figsize=(9, 5))          #Plotting the graphs
plt.hist(y, bins=30)
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=1)        #dividing the data into training and testing
alg = LinearRegression()                                #calling the algorithm
alg.fit(X_train, y_train)                               #Fitting the data
filename = 'coimbatore.pkl'  #name the file 
...
print(coef)
print()
y_predict = alg.predict(X_test)                 #prediction of value
